                ...
                <div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-1.5">
                  {sortedImages.map((_, idx) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all ${idx === imgIndex ? 'w-5 bg-white' : 'w-1.5 bg-white/50'
                        }`}
                    />
                  ))}
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[1, 2, 3, 4, 5].map((value) => (
                      <button
                        ...
                        className={`h-10 w-10 rounded-lg border text-sm font-medium transition-colors ${item.score === value
